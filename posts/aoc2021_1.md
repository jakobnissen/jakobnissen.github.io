...
With your open REPL, navigate to the `AoC2021` directory you just created (type ';' to enter shell mode), enter package mode and execute `activate` to have the package manager activate the project.
...
We also add a new `day1.jl` file in the `src` directory for today's code, and make the toplevel project file `AoC2021.jl` file `include` the new file.
...
and also import it at the top of the `day1` file using a relative import (so it imports it from the top-level module):
