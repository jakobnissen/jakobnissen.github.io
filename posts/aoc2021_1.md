...
julia> @btime open(AoC2021.day01.solve, "data/day01.txt");
  251.896 μs (4021 allocations: 179.92 KiB)
```
@@
...
Benchmarking only the `solve(::AbstractVector)` method by itself reveals that the optimization changed the minimum run time from 4.68 μs to 3.15 μs.
A decent improvement on the numerical part, but insignificant compared to the parse cost.
