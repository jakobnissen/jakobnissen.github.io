...
To solve the 24 Advent of Code puzzles, We _could_ just make a series of scripts, but let's do it the _right_ way and instead create a new Julia project for this year's AoC. To do this, launch Julia, then type `]` to enter `Pkg` mode, then type:
...
With your open REPL, navigate to the `AoC2021` directory you just created (type ';' to enter shell mode), enter package mode and execute `activate` to activate the project.
...
We also add a new `day1.jl` file in the `src` directory for today's code, and make the top-level project file `AoC2021.jl` file `include` the new file.
...
Here, for example, I analyze how the main function behaves when called with an `IOBuffer`:
...
## Benchmarking
Wise programmers say that, most of the time, slow code is fast enough, and developers should optimize their code for maintainability instead of speed. That may be true, but that has never stopped me from over engineering otherwise uncomplicated software until it goes _brrrrrr_.

We'll use the `BenchmarkTools` package.
